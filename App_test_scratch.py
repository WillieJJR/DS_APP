...
button_linear_regression = html.Button("Linear Regression Model", id="linearregression-button",
                                                n_clicks=0,
                                                n_clicks_timestamp=0,
                                                style={
                                                    'backgroundColor': 'rgba(0,0,0,0.3)',
                                                    ...
                                                    'border-radius': '7%',
                                                    # set the border radius to 50% to make the buttons round
...
button_rf_regress = html.Button("Random Forest Model", id="randomforest-button",
                                    n_clicks=0,
                                    n_clicks_timestamp=0,
                                    style={
                                        'backgroundColor': 'rgba(0,0,0,0.3)',
...
button_rf_class = html.Button("Random Forest Model", id="rfclass-button",
                                    n_clicks=0,
                                    n_clicks_timestamp=0,
                                    style={
                                        'backgroundColor': 'rgba(0,0,0,0.3)',
...
button_regression_container = html.Div(
    children=[button_linear_regression, button_rf_regress],
    style={'display': 'flex', 'margin': 'auto', 'justify-content': 'center'}
)

button_classification_container = html.Div(
    children=[button_svm, button_rf_class],
    style={'display': 'flex', 'margin': 'auto', 'justify-content': 'center'}
)
    ...
    dbc.Tabs([
        ...
        dbc.Tab(label='Predictive Analytics', children=[
            html.Div([
                ...
                dbc.Row([
                    html.Div(id="button_regression_container", children=[
                        button_linear_regression, button_rf_regress
                    ], style={"display": "none"}),
                    html.Div(id="button_classification_container", children=[
                        button_svm, button_rf_class
                    ], style={"display": "none"})
                ]),
                dbc.Row([
                    dbc.Col(
...
def show_popover(n_clicks_regress, n_clicks_class):
    if n_clicks_regress % 2 == 1:
        ...
        return dbc.Popover(
            ...
            * Classification algorithm that finds a decision boundary (line or hyperplane) that separates different classes of data with the largest margin.
            
            ### ROC Curve
            * Graphical representation of the performance of a binary classifier. It plots the true positive rate (TPR) against the false positive rate (FPR) at different classification thresholds.
            
            ### Random Forest Classifier
            * An ensemble machine learning model, built upon decision trees, that is used to perform classification tasks. The model makes predictions by averaging the results of many decision trees, where each tree is trained on a random subset of the data.
            """)),
            target='button-regression-help',
            trigger="hover"
        )
...
@app.callback(
    Output("button_regression_container", "style"),
    [Input("button-regression", "n_clicks"), Input("button-classification", "n_clicks"), Input("button-placeholder", "n_clicks")],
    [State("button_regression_container", "style")],
)
def show_regression_container(n_clicks_regress, n_clicks_class, n_clicks_placeholder, style):
    if n_clicks_regress is None or (n_clicks_class % 2 == 1 or n_clicks_placeholder % 2 == 1):
        style["display"] = "none"
    elif n_clicks_regress % 2 == 1 and n_clicks_class % 2 == 0 and n_clicks_placeholder % 2 == 0:
        style["display"] = "block"
...
@app.callback(
    Output("button_classification_container", "style"),
    [Input("button-regression", "n_clicks"), Input("button-classification", "n_clicks"), Input("button-placeholder", "n_clicks")],
    [State("button_classification_container", "style")],
)
def show_classification_container(n_clicks_regress, n_clicks_class, n_clicks_placeholder, style):
    if n_clicks_class is None or (n_clicks_regress % 2 == 1 or n_clicks_placeholder % 2 == 1):
        style["display"] = "none"
    elif n_clicks_class % 2 == 1 and n_clicks_regress % 2 == 0 and n_clicks_placeholder % 2 == 0:
        style["display"] = "block"
        #style["justify-content"] = "center"
    else:
        style["display"] = "none"
...
def update_classification_graph(n_clicks_class, n_clicks_svm, n_clicks_rfclass, jsonified_cleaned_data, feature_columns, target_column):
    if n_clicks_class is not None and n_clicks_svm% 2 == 1:
        ...
        button_container = html.Div(
            children=[button1, button2, button3],
            style={'display': 'flex', 'margin': 'auto', 'justify-content': 'center'}
            # set the display property to flex to arrange the buttons horizontally
        )
        ...
        app.layout = html.Div([
            ...
            dbc.Tabs([
                dbc.Tab(label='Home', children=[
                    ...
                    html.Div(
                        children='Note: Files containing over 10,000 records will be truncated to 10,000 records via Random Sampling for all features in the app.',
                        style={
                            'border': '2px solid red',
                            'border-radius': '4px',
                            'padding': '10px',
                ...
                dbc.Tab(label='Feature Exploration', children=[
                    ...
                    html.Div([
                        html.Div([
                            html.Center(
                                html.H2(
                                    'Understand Your variables! Please select a technique to better explore your data.')
                            )
                        ], className="row"),
                        ...
                        dbc.Row(html.Div(id='warning-message',
                                         style={'color': 'red', 'fontSize': 20, 'text-align': 'center'})),
                        dbc.Row(html.Div(id='scatterplot-div', children=[
                            html.Div(id='scatter-plot')
                        ])),
                        dbc.Row(html.Div(id='dist-plot-div', children=[
                ...
                dbc.Tab(label='Predictive Analytics', children=[
                    html.Div([
                        ...
                        dbc.Row(html.Div(id='warning-message-regression',
                                         style={'color': 'red', 'fontSize': 20, 'text-align': 'center'})),
                        dbc.Row(html.Div(id='regression-div', children=[
                            html.Div(id='regression-plot'),
                            html.Div(id="regression-results", style={"display": "none"}),
            ...
            if contents:
                ...
                if value is not None:
                    ...
                    if df[value].dtypes in (numbs):
                        ...
                        data_describe = df[value].mode()[0]
                        data_describe_mode = round((df[value].value_counts().loc[data_describe] / len(df[value])) * 100,
                                                   2)
                        return f'''Mode:    {data_describe} ({data_describe_mode} % of column values) '''
                    else:
                        return f'''Neither a Mode or Median can be applied to this datatype.'''
        ...
        def toggle_dropdown_visibility_x_var_classification(button_regression_click, svm_button, rfclass_button):
            if (button_regression_click is None) or (button_regression_click % 2 == 0):
                ...
                if (svm_button is not None or rfclass_button is not None) and (
                        svm_button % 2 == 1 or rfclass_button % 2 == 1):
                    # Make the dropdown visible when Button 1 is clicked an odd number of times
                    return [
                        {'display': 'block', 'color': 'black', 'textAlign': 'center'},
                        None
        ...
        def update_scatterplot_visibility(n_clicks_1, n_clicks_2, n_clicks_3):
            if (n_clicks_3 is not None and n_clicks_3 % 2 == 1) and (n_clicks_2 is None or n_clicks_2 % 2 == 0) and (
                    n_clicks_1 is None or n_clicks_1 % 2 == 0):
                # Make the featimp visible when button 3 is clicked an odd number of times and all other buttons are clicked an even number of times
                return {'display': 'block'}
            else:
                # Make the scatterplot invisible in all other cases
        ...
        @app.callback(
            ...
            [Input("button-regression", "n_clicks"), Input("button-classification", "n_clicks"),
             Input("button-placeholder", "n_clicks")],
        )
        ...
        def update_scatterplot(x_axis, y_axis, jsonified_cleaned_data, n_clicks):
            if n_clicks is not None:
                if (jsonified_cleaned_data is not None) and (x_axis is not None) and (y_axis is not None):
                    ...
                    figure = px.scatter(df, x=x_axis, y=y_axis,
                                        title=f'''Scatter Plot: Relationship between {x_axis} and {y_axis}''')
                    figure.update_traces(marker=dict(color='red'))
                    figure.update_xaxes(showgrid=False)
                    figure.update_yaxes(showgrid=False)
                    figure.update_layout({
                    ...
                    return html.Div([
                        html.Center(
                            html.H4('Please make sure to select BOTH an X and Y variable to display Scatterplot')),
                        dcc.Loading(type='circle', children=[html.Div(id='loading-scatterplot')])
                    ])
                ...
                if (jsonified_cleaned_data is not None) and (target_value is not None):
                    ...
                    if df[target_value].dtype == 'object':
                        if df[target_value].nunique() > 5:
                            return html.Div([
                                html.Center(html.H2(
                                    'This target variable has a high degree of cardinality. This will not likely derive any meaningful insights.')),
                                dcc.Loading(type='circle', children=[html.Div(id='loading-corrplot')])
                            ])
                        else:
                            le_encoder = LabelEncoder()
        ...
        def update_regression_graph(n_clicks_regress, n_clicks_linear, n_clicks_rf, jsonified_cleaned_data,
                                    feature_columns,
                                    target_column):
            if n_clicks_regress is not None and n_clicks_linear % 2 == 1:
                if (jsonified_cleaned_data is not None) and (target_column is not None) and (
        ...
        def update_classification_graph(n_clicks_class, n_clicks_svm, n_clicks_rfclass, jsonified_cleaned_data,
                                        feature_columns,
                                        target_column):
            if n_clicks_class is not None and n_clicks_svm % 2 == 1:
                if (jsonified_cleaned_data is not None) and (target_column is not None) and (
                ...
                if (jsonified_cleaned_data is not None) and (target_column is not None) and (
                    ...
                    y_encoded = le.fit_transform(y)
                    # testing
                    class_dict = dict(zip(le.classes_, le.transform(le.classes_)))
                    print(class_dict)
                    # testing
                    # split data
                    X_train, X_test, y_train, y_test = train_test_split(X, y_encoded, random_state=10)
                    ...
                    if n_classes <= 2:
                        ...
                        for label in labels:
                            ...
                            fig.add_trace(go.Scatter(x=fpr, y=tpr,
                                                     mode='lines',
                                                     name='ROC curve for class {} (AUC = {:.3f})'.format(
                                                         le.inverse_transform([label])[0],
                                                         roc_auc)))  # name='ROC curve for class {} (AUC = {:.3f})'.format(label, roc_auc)))
